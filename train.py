...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='mcc')
...
def prepare_training(args: ArgsType):
    ...
    #   Load Data
    # ----------------------------------------------
    train_set = CarlaDataset(carla_root="data/train/Town05", categories=args.categories)
    # train_set = CarlaDataset(carla_root="data/train/Design", categories=args.categories)
...
def train():
    ...
    args.save_dir = os.path.join("temps","train", f"{args.save_dir}-{'_'.join(args.categories)}-{nowt}")
