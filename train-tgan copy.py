...
def train():
    ...
    n_r = 0.01
    x_n = (1 - n_r) * x_t + n_r * torch.rand_like(x_t)
    ...
    for epoch in range(args.epochs):
        ...
        if True:
            for i_mb, batch_data in enumerate(pbar):
                ...
                fake_xs = model.decode(model.forward(x_n, coco_label))
                ...
                if i_mb%4==0:
                    loss_smooth = 0.05 * F.l1_loss(x_t, fake_xs)
        ...
        if epoch%10==0:
            ...
            with torch.no_grad():
                ...
                _t = tt.repeat(x_t.shape[0], *[1] * (len(tt.shape) - 1))
                _t[:, neural_renderer.selected_faces, :] = x_t
                real_textures = _t

                _t = tt.repeat(x_t.shape[0], *[1] * (len(tt.shape) - 1))
                _t[:, neural_renderer.selected_faces, :] = torch.rand_like(x_t).to(x_t.device)
                noise_textures = _t
