...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='tmp/tests')
    parser.add_argument('--save_name', type=str, default='autoencoder')

    parser.add_argument('--pretained', type=str, default=None)
    ...
    for iter in pabr:
        ...
        if iter % args.save_interval == 0:
            ...
            merged_img = cv2.hconcat(
                [render_a_image(neural_renderer, image, X) for X in (
                    x_t[0].squeeze(0),
                    x_in[0].squeeze(0),
                    x_rec[0].squeeze(0),
                )]
