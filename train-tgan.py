...
def prepare_training(args: ArgsType):
    ...
    #   Load Data
    # ----------------------------------------------
    train_set = tsgan.data.CarlaDataset(carla_root="tmp/data", categories=args.categories)
    train_loader = data.DataLoader(
        train_set,
        batch_size=args.batch,
        num_workers=args.num_workers,
        collate_fn=tsgan.data.CarlaDataset.collate_fn,
        drop_last=True
    )
    ...
    model = TextureGenerator(
        ...
        cond_dim=len(train_set.coco_ic_map),
    ...
    return (neural_renderer, model, detector, detector_loss, optimizer, lr_heduler, train_set, train_loader)
...
def train():
    ...
    (neural_renderer, model, detector, detector_loss, optimizer, lr_heduler, train_set,
     train_loader) = prepare_training(args)
    device = "cuda"
    tt = neural_renderer.textures[:, neural_renderer.selected_faces, :]

    n_r = 0.01
    x_t = tt.repeat(args.batch, *[1] * (len(tt.size()) - 1))
    ...
    for epoch in range(args.epochs):
        ...
        if True:
            for i_mb, items in enumerate(pbar):
                ...
                x_adv = model.decode(model.forward(x_i, labels))

                adv_textures = neural_renderer.textures.repeat(
                    x_adv.shape[0], *[1] * (len(neural_renderer.textures.size()) - 1)
                )
                adv_textures[:, neural_renderer.selected_faces, :] = x_adv
                ...
                for i_b in range(bs):
                    ...
                    render_img = np.ascontiguousarray(
                        render_image.squeeze(0).detach().cpu().numpy().transpose(1, 2, 0) * 255
                    ).astype(np.uint8)
                ...
                if i_mb % 4 == 0:
                    loss_smooth = 0.05 * F.l1_loss(x_t, x_adv)
                    loss_smooth_epoch += loss_smooth.item() * bs / 4
                    show_ls = loss_smooth.item()
                else:
                    loss_smooth = 0
                    show_ls = loss_smooth
        ...
        if epoch % 10 == 0:
            ...
            pbar = tqdm.tqdm(random.sample(train_set, 9))
            result_imgs = []
                    ...
                    with torch.no_grad():
                        ...
                        if len(pred_results):
                            for *xyxy, conf, cls in pred_results:
                                ...
                                cv2.putText(
                                    render_img, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
                                )
                    return render_img

                clean_img = get_pred_img(x_t)
                attack_img = get_pred_img(x_adv)
                noise_img = get_pred_img(x_n)
                result_imgs.append(cv2.vconcat([clean_img, attack_img, noise_img]))
