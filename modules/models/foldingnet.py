...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channel=512):
        ...
        self.grid = np.array(self.grid) # Only for accelerating

        # reshape
        self.grid = torch.Tensor(self.grid).view(2, -1) # (2, 45, 45) -> (2, 45 * 45)

        self.m = self.grid.size(1)
        print("self.m", self.m)
    ...
    def forward(self, x: Tensor):
        ...
        recon1 = self.fold1.forward(grid, x)
        print("recon1", recon1.size())
        recon2 = self.fold2.forward(recon1, x)
        print("recon2", recon2.size())
