...
def train(args, ):
    ...
    loader = data.DataLoader(
        ...
        sampler=data_sampler(croppedcoco_train_set, shuffle=True, distributed=args.distributed),
        drop_last=True,
    )
    ...
    cond_model.fc = nn.Linear(
        ...
        croppedcoco_train_set.COCO_CATEGORIES_MAP.__len__(),
    )
    ...
    for idx in pbar:
        ...
        batch_dict = next(loader)
        real_img = batch_dict["image"].to(device)
        images = batch_dict["image"].to(device)
        categories = batch_dict["predict_id"].to(device)
        ...
        with torch.no_grad():
            cond_latent = cond_model.forward_latent(images)
