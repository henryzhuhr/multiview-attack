...
def render_image():
    ...
    # ==========================
    encoder = TextureEncoder(texture_size=4, zdim=512)
    encoder.to(args.device)
    # decoder = PointwiseNet(out_dim, zdim=512)
    encoder.to(args.device)
    optimizer = optim.SGD([{'params': encoder.parameters(), 'lr': 1e-4}, {'params': decoder.parameters(), 'lr': 1e-4}])
    encoder.train()
    for i in range(1000):

        x = neural_renderer.textures
        optimizer.zero_grad()
        mean, logvar = encoder.forward(x)
