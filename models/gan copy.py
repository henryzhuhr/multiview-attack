...
class Generator(nn.Module):
    def __init__(
        ...
        self.embedding_layer = nn.Sequential(nn.Linear(cond_dim, style_dim),nn.LeakyReLU(0.2))
        ...
        self.out_mapping_layer = nn.Linear(size * 3, latent_dim)
...
class TextureGenerator(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.cond_dim = cond_dim
        self.encoder = TextureEncoder(nt=nt, sample_point=sample_point, ts=ts, dim=style_dim)
        self.decoder = TextureDecoder(nt=nt, ts=ts, dim=style_dim)
        self.generator = Generator(style_dim=style_dim, cond_dim=cond_dim, mix_prob=mix_prob)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor = None):
        ...
        rec = self.generator.forward(latent, y)
        return rec
