...
def retrieve_from_label(label_file: str):
    ...
    try:
        ...
        rgb_camera_actor: types.carla.Actor = world.spawn_actor(
            CameraManager(blueprint_library).get_camera_rgb_blueprint(),
            camera_transform,
        ...
        abs_transform = rgb_camera_actor.get_transform()
        rgb_camera_actor.stop()

        with open(label_file, 'w') as f:
            # label_dict['state'] = state# 改变状态
            label_dict['camera_abs'] = {
                "location":
                    {
                        "x": abs_transform.location.x,
                        "y": abs_transform.location.y,
                        "z": abs_transform.location.z
                    },
                "rotation":
                    {
                        "pitch": abs_transform.rotation.pitch,
                        "yaw": abs_transform.rotation.yaw,
                        "roll": abs_transform.rotation.roll,
                    },
            }
            json.dump(
