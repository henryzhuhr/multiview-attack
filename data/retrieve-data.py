...
def retrieve_from_map(world_map: str):
    ...
    try:
        ...
        rgb_camera_manager = CameraManager(world, camera_type=CameraManager.CameraTypes.RGB)
        iseg_camera_manager = CameraManager(world, camera_type=CameraManager.CameraTypes.ISeg)

        vehicle_actor: types.carla.Actor = world.spawn_actor(vehicle_bp, spawn_points[0])
        if Settings.autopilot:
            vehicle_actor.set_autopilot(True)
        rgb_camera_manager.spawn_attach_to(vehicle_actor)
        iseg_camera_manager.spawn_attach_to(vehicle_actor)
        print(vehicle_actor)
        print(rgb_camera_manager.actor)
        print(iseg_camera_manager.actor)
        actor_list.append(vehicle_actor)
        actor_list.append(rgb_camera_manager.actor)
        actor_list.append(iseg_camera_manager.actor)
        ...
        for spawn_point_index, spawn_point in enumerate(spawn_points):
            ...
            for distance_index, (x, y, z, fov) in enumerate(Settings.camera_distances):
                for direction_index, (x, y, z) in enumerate(Utils.extend_xyz(x, y, z)):
                    ...
                    rgb_camera_manager.transform_to(camera_transform)
                    iseg_camera_manager.transform_to(camera_transform)
                    time.sleep(2)

                    rgb_camera_manager.actor.listen(
                    ...
                    iseg_camera_manager.actor.listen(
                        lambda image: CameraManager.save_img(
                            image,                                                                      #
                            [CameraManager.Settings.image_size_x, CameraManager.Settings.image_size_y],
                    ...
                    # rgb_camera_manager.actor.stop()
                    # carla.command.DestroyActor(rgb_camera_actor)

                    # iseg_camera_manager.actor.stop()
...
class CameraManager:
    """
    - `camera_type`: `rgb`, `instance_segmentation`
    """
    class CameraTypes:
        RGB = 'rgb'
        ISeg = 'instance_segmentation'
    ...
    def __init__(self, world, camera_type=CameraTypes.RGB) -> None:
        self.world = world
        self.blueprint_library = world.get_blueprint_library()
        self.camera_type = camera_type
        self.bp = self.__set_blueprint()
        self.actor: types.carla.Actor = None

    def __set_blueprint(self):
        bp = self.blueprint_library.find(f'sensor.camera.{self.camera_type}')
        # common settings
        ...
        if self.camera_type == self.CameraTypes.RGB:
            bp.set_attribute('shutter_speed', str(self.Settings.shutter_speed))
        return bp

    def spawn(self):
        actor: types.carla.Actor = self.world.spawn_actor(
            self.bp,                                                                             #
            carla.Transform(location=carla.Location(0, 0, 0), rotation=carla.Rotation(0, 0, 0)),
        )
        self.actor = actor

    def spawn_attach_to(self, vehicle_actor):
        actor: types.carla.Actor = self.world.spawn_actor(
            self.bp,                                                                             #
            carla.Transform(location=carla.Location(0, 0, 0), rotation=carla.Rotation(0, 0, 0)),
    ...
    def transform_to(self, transform: types.carla.Transform):
