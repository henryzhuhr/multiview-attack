...
def get_args():
    ...
    parser.add_argument('--data_dir', type=str, default="tmp/pics/Town10HD-0613_1109-dog")
...
def main():
    ...
    for i_d, label in enumerate(pbar):
        ...
        cls_count = {} # 对每一张图 iou 中统计类别，以证明一致性
        for attack_name, file in label['imgs'].items():
            ...
            if not is_detected:
                cls_count[attack_name] = -1

        reuslt_list.append(correct)
        class_result_list.append(cls_count)
    ...
    class_count = {n: {} for n in attack_names}
    for item in class_result_list:
        for n, c in item.items():
            if c == -1:
                cls_name = "None"
            else:
                cls_name = CarlaDataset.coco_ic_map[c]

            if cls_name not in class_count[n]:
                class_count[n][cls_name] = 0
            class_count[n][cls_name] += 1
    with open(f"{data_dir}-{args.model}-result.json", "w") as f:
        json.dump({"model": args.model, "ap": ap_dict, "class": class_count}, f, indent=4)
